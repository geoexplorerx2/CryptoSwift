...
class CoinPaymentsAPI
{
    ...
    /**
     * Creates a basic transaction with minimal parameters.
     * See CreateTransaction for more advanced features.
     * @param amount The amount of the transaction (floating point to 8 decimals).
     * @param currency1 The source currency (i.e., USD), used to calculate the exchange rate for you.
     ...
     * @param buyer_email Set the buyer's email for refunds if there is an issue with their payment.
     * @param address Optionally set the payout address of the transaction. If empty, it follows your payout settings for that coin.
     * @param ipn_url Optionally set an IPN handler to receive notices about this transaction. If empty, it uses the default IPN URL in your account.
     */
    ...
    private function api_call($cmd, $req = array())
    {
        ...
        if ($data !== FALSE) {
            if (PHP_INT_SIZE < 8 && version_compare(PHP_VERSION, '5.4.0') >= 0) {
                // We are on 32-bit PHP, so use the bigint as a string option. If you are using any API calls with Satoshis, it is highly NOT recommended to use 32-bit PHP
